...
| Navigable Paths | Cartography, Pheromones, Drift, Traces | **Geklaert 2026-06-24** unter `adr/adr-003-ai-as-maintainer.md`: Cartography bestaetigt (indirekt via `genesis-scope`-Domain-Text, P39), bleibt offene Semantic-Maps-Linie. Pheromones/Drift/Traces kommen in `ECOSYSTEM_MAP.yaml` (48 Pakete) ueberhaupt nicht vor und wurden als reine Genesis-Diskursterminologie nach `archive/navigable-paths-pheromones-drift-traces-diskursbegriffe` verschoben (Regel 9/11, vollstaendig revidierbar). Befund-Recherche: `01_Ideen/claude/cartography-pheromones-drift-ecosystem-map-befund`. |
